...
for it = 1:nIter
    ...
    tLS = tic;
    if useGPU
        Phi_y_gpu = gpuArray(Phi_y);
        x_gpu     = gpuArray(x_target);
        c_gpu = (Phi_y_gpu' * Phi_y_gpu) \ (Phi_y_gpu' * x_gpu);
        c_mp = gather(c_gpu);            % bring back to CPU (MATLAB uses CPU PA model)
    else
        c_mp = (Phi_y' * Phi_y) \ (Phi_y' * x_target);
    end
    fprintf('  LS solve time = %.3f sec\n', toc(tLS));
